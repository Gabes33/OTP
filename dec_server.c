  ...
  // Get the size of the address for the current client that will connect
    ...
    if (pid == 0) {
      ...
      if (clientConnectionConfirm(connectionSocket)) {
        ...
        fprintf(stderr, "The connection to the client socket is invalid\n");
        exit(1);
        break;
      }
      exit(0);
...
char msgBuff[] - message buffer that will receive incoming message
...
void decMsg(char message[], char key[], int size) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (msgChar != 10) {
      ...
      //correct capitol letter as a ASCII value. This is then converted back to a character and assigned to the correct inedex o the
      //message buffer

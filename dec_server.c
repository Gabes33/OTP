  ...
  // Get the size of the address for the current client that will connect
    ...
    if (pid == 0) {
      if (clientConnectionConfirm(connectionSocket)) {
        ...
        // The message buffer with the decrypted message is sent to the client until
        // charsSent is equal to the fileSize variable. The length of the message buffer
        // is kept track of through each iteration. Once all bytes are sent, the child
        // process exits
...
int clientConnectionConfirm(int socket) {
  ...
  while (char_rcv == 0) {
    ...
    if ((strcmp(incoming_buff, "dec_val")) == 0) {
...
void encMsg(char message[], char key[], int size) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (msgChar != 10) {
      ...
      // If the character calculation result is less than 0, we will add 27 to get the correct numerical value
      if (msgChar < 0) {
        msgChar += 27;
      }
      ...
      //the space character. If the integer is 26, the numerical value is a space character. Otherwise, we
      //take the converted integer and add the start letter, which ASCII value is 65, to it to get the correct capitol
      //letter as a ASCII value. This is then converted back to a character and assigned to the correct inedex of the message
      //buffer
...
to be the correct value for decrypting the message
